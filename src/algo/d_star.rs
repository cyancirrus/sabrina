    ...
    S::Encoded: Eq + Hash + std::fmt::Debug + Eq,
{
    ...
    fn compute_shortest_path(&mut self, env: &S) {
        ...
        loop {
            ...
            if u_coord == env.encode((5,3)) && source == u_coord {
                // assert!(false);
                println!("Checking what the keys are");
                println!("k_new {k_new:?}");
                println!("k_old {k_old:?}");
            }
            // reversed due to starkey reversed compare for order for minheap
            if k_old > k_new {
                println!("pushing due to k_old > k_new {:?}", u_coord);
    ...
    S::Encoded: Eq + Hash + std::fmt::Debug + Eq,
{
    ...
    fn plan(&mut self, env: &S, source: Coord, target: Coord) -> Option<Self::Plan> {
        ...
        if self.source.is_none() || self.target.is_none() || self.origin.is_none() {
            ...
            println!("IN PLAN STAR LENGTH {:?}", self.star.len());
            let s_encode = env.encode(source);
            if s_encode == env.encode((5,3)) {
                let (g, rhs) = self.star[&s_encode];
                println!("The distance is {g:?}, {rhs:?}");
